...
#define pgm_read_dword(x)    (*(const int*)x)
...
TSD305::TSD305(PinName sda, PinName scl) 
{
...
enum TSD305_status TSD305::read_eeprom_coeff(uint8_t address, uint16_t *coeff) 
{
  ...
  i2c_->write((TSD305_ADDR << 1) & 0xFE, tx, 1);
  
  wait_ms(1);
  
  
  i2c_->read((TSD305_ADDR << 1) | 0x01, rx, 3);
...
enum TSD305_status TSD305::conversion_and_read_adcs(uint32_t *adc_object, uint32_t *adc_ambient) 
{
  ...
  tx[0] = TSD305_CONVERT_ADCS_COMMAND;
  
  
  i2c_->write((TSD305_ADDR << 1) & 0xFE, tx, 1);

  
  wait_ms(TSD305_CONVERSION_TIME);

  i2c_->read((TSD305_ADDR << 1) | 0x01, rx, 7);
  ...
  *adc_object = ((uint32_t)rx[1] << 16) | ((uint32_t)rx[2] << 8) | (uint32_t)rx[3];
  *adc_ambient = ((uint32_t)rx[4] << 16) | ((uint32_t)rx[5] << 8) | (uint32_t)rx[6];
